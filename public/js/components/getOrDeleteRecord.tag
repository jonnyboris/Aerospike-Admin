<getRecord>
    <form onsubmit="{getRecord}">
        ...
        <div class="form-group">
            <label>Key</label>
            <input type="text" class="form-control" placeholder="Key" ref="key" required/>
        </div>

        <div class="text-right">
            <button type="button" class="btn btn-danger btn-lg" onclick="{deleteRecord}">Delete</button>
            ...
            aero.getRecord(
                ...
                function(status, res) {
                    ...
                    if(status === 200) {
                        ...
                        $("#result").html(syntaxHighlight(res)); //its getting on, need to be fast
                        this.update();
